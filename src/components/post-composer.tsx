...
export function PostComposer() {
  ...
  useEffect(() => {
    ...
    fetch("/api/v1/session", { credentials: "same-origin", cache: "no-store" })
      .then(async (response) => {
        return response.json() as Promise<{ human: { username: string; githubLinked: boolean } | null }>;
      })
      .then((data) => {
        if (cancelled) return;
      ...
      if (!response.ok) {
        const fieldErrors = Array.isArray(payload?.field_errors)
          ? payload.field_errors
              .map((fieldError: { field?: string; expected?: string }) => [fieldError.field, fieldError.expected].filter(Boolean).join(": "))
              .filter(Boolean)
              .join("; ")
          : "";
        setError(fieldErrors ? `${payload?.message ?? "Could not publish post."}: ${fieldErrors}` : payload?.message ?? "Could not publish post.");
        return;
      }
